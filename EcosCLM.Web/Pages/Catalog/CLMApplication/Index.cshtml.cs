...
{
    public class IndexModel : BasePageModel<CLMApplicationViewModel>
    {
        ...
        public CLMApplicationViewModel Search { get; set; }
        ...
        private void AddGridConfig()
        {
            ...
            {
                Title = "CLM Applications",
                AddPageUrl = "Add",
                SearchPlaceholder = "Name or Code",
                SearchQuery = Search?.Name ?? string.Empty,
                CurrentPage = Pager?.CurrentPage ?? 1,
                TotalPages = Pager?.TotalPages ?? 1,
                Headers = new List<string> { "Code", "Name", "Criticality", "Status", "Created At" }
            };
        }
        ...
        private async Task GetData()
        {
            try
            {
                ...
                {
                    Itens = new List<CLMApplicationViewModel>();
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError(ex, "Error processing application list data.");
            }
        }
    }
}