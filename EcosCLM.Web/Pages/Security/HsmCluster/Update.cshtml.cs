...
{
    public class UpdateModel : BasePageModel<HsmClusterViewModel>
    {
        ...
        public async Task<IActionResult> OnGetAsync(Guid id)
        {
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError(ex, "Error retrieving HSM cluster for update.");
                return RedirectToPage("Index");
            }
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    existingEntity.Model = Item.Model;
                    existingEntity.PartitionLabel = Item.PartitionLabel;
                    existingEntity.EndpointRef = Item.EndpointRef;
                    existingEntity.FipsLevel = Item.FipsLevel;
                    existingEntity.Status = Item.Status;
                    existingEntity.UpdatedAt = DateTime.UtcNow;

                    await _repository.UpdAsync(existingEntity).ConfigureAwait(false);

                    TempData["success"] = "HSM Cluster updated successfully!";
                    return RedirectToPage("Index");
                }
                catch (Exception ex)
                {
                    TempData["error"] = ex.Message;
                    _logger.LogError(ex, "Error updating HSM cluster.");
                    return Page();
                }
            }
