...
{
    public class UpdateModel : BasePageModel<CertificateProfileViewModel>
    {
        ...
        public async Task<IActionResult> OnGetAsync(Guid id)
        {
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError(ex, "Error retrieving certificate profile for update.");
                return RedirectToPage("Index");
            }
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    existingEntity.CertificateType = Item.CertificateType;
                    existingEntity.KeyAlgorithm = Item.KeyAlgorithm;
                    existingEntity.KeySize = Item.KeySize;
                    existingEntity.CurveName = Item.CurveName;
                    existingEntity.SignatureAlgorithm = Item.SignatureAlgorithm;
                    existingEntity.ValidityDays = Item.ValidityDays;
                    existingEntity.RenewalWindowDays = Item.RenewalWindowDays;
                    existingEntity.RequireApproval = Item.RequireApproval;
                    existingEntity.Status = Item.Status;
                    existingEntity.UpdatedAt = DateTime.UtcNow;

                    await _repository.UpdAsync(existingEntity).ConfigureAwait(false);

                    TempData["success"] = "Certificate Profile updated successfully!";
                    return RedirectToPage("Index");
                }
                catch (Exception ex)
                {
                    TempData["error"] = ex.Message;
                    _logger.LogError(ex, "Error updating certificate profile.");
                    return Page();
                }
            }
