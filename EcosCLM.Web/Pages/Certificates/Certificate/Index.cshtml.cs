...
{
    public class IndexModel : BasePageModel<CertificateViewModel>
    {
        ...
        public CertificateViewModel Search { get; set; }
        ...
        private void AddGridConfig()
        {
            GridConfig = new GridConfiguration
            {
                Title = "Certificates",
                AddPageUrl = "Add",
                SearchPlaceholder = "Subject DN or Serial Number",
                SearchQuery = Search?.SubjectDn ?? string.Empty,
                CurrentPage = Pager?.CurrentPage ?? 1,
                TotalPages = Pager?.TotalPages ?? 1,
                Headers = new List<string> { "Subject DN", "Serial Number", "Expiration", "Status", "Created At" }
            };
        }
        ...
        private async Task GetData()
        {
            try
            {
                ...
                {
                    Itens = new List<CertificateViewModel>();
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError(ex, "Error processing certificate list data.");
            }
        }
    }
}